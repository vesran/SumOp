...
def segment_corpus(corpus, segmenter, window_size, stops):
    ...
    # Merge
    fragments = merge_segments_and_sentiments(units, sentiments)
    return fragments


def merge_segments_and_sentiments(segments, sentiments):
    fragments = []
    for seg_group, senti_group in zip(segments, sentiments):
        for text, sentiment in zip(seg_group, senti_group):
            fragments.append((text, sentiment))
    return fragments
...
def complete_prediction(ndocs, window_size):
    for i in range(len(ndocs)):
        ndocs[i] = complete_prediction_single_ndoc(ndocs[i], window_size)
    return ndocs
